...
All text in Korean.`;
    ...
    const [res1, res2] = await Promise.all([
      fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model, max_tokens: 2000, temperature: 0.7, system: systemPrompt,
...
verdict values: 치명 리스크, 수정 권장, 검토 필요, 양호. All text in Korean.`;
      ...
      const rSafe = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
        body: JSON.stringify({ model, max_tokens: 1000, temperature: 0.7, system: systemPrompt,
...
Return ONLY valid JSON: {"subtitle":"YES","title":"YES","button":"YES","mainImage":"YES"}`;
      ...
      const [rZa, rZb] = await Promise.all([
        fetch('https://api.anthropic.com/v1/messages', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
          body: JSON.stringify({ model, max_tokens: 300, temperature: 0.7, system: systemPrompt,
            messages: [{ role: 'user', content: [imageContent, { type: 'text', text: coordPrompt }] }] })
        }),
        fetch('https://api.anthropic.com/v1/messages', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
          body: JSON.stringify({ model, max_tokens: 200, temperature: 0.7, system: systemPrompt,
            messages: [{ role: 'user', content: [
              { type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: overlayBase64 } },
